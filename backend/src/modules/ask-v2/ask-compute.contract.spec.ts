...
describe('Ask compute compatibility contract', () => {
  ...
  test.each([
    ...
    ['UNKNOWN', true],
  ])('confirmation boundary for %s is %s regardless of charging', (workClass, expected) => {
    expect(requiresExplicitAcceptance(workClass as string)).toBe(expected);
  });
  test.each([
    { deepRequested: true },
    { reportRequested: true },
    { deepRequested: true, reportRequested: true },
  ])('rejects contradictory contextual planning %j', (flags) => {
    expect(() => validatePlan({ ...plan, contextual: true, ...flags })).toThrow('ASK_PLAN_INVALID');
  });
  test.each(['deep-analysis', 'research-report'] as const)(
    'rejects contextual plan against explicit %s intent',
    (intent) => {
      expect(() => validatePlan({ ...plan, contextual: true }, { ...request, intent })).toThrow(
        'ASK_PLAN_INVALID',
      );
    },
  );
  test('wide contextual coverage alone does not imply new deep execution', () => {
    const contextual = {
      ...plan,
      contextual: true,
      countryCount: 10,
    ...
    expect(() => validatePlan(contextual, request)).not.toThrow();
    expect(classifyCompute(request, contextual, false)).toBe('CONTEXTUAL');
  });
  test.each(['deep-analysis', 'research-report'] as const)(
    'stored %s reuse dominates fresh work and confirmation',
    (intent) => {
      const workClass = classifyCompute({ ...request, intent }, plan, true);
      expect(workClass).toBe('STORED');
      expect(requiresExplicitAcceptance(workClass)).toBe(false);
    },
  );
