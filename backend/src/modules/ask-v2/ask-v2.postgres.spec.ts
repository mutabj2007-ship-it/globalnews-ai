...
live('Ask V2 PostgreSQL durability, lifecycle and HTTP authorization', () => {
  ...
  test('quote, acceptance and reservation are distinct; no execution before acceptance', async () => {
    const op = await quoted();
  ...
  test('expired quotes reject acceptance and reserved expiry releases', async () => {
    const op = await quoted();
