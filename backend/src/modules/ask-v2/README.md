...
Use `POST threads/:id/turns` for conversational submission with the same DTO and
idempotency semantics as quote. STORED, CONTEXTUAL and FRESH_BOUNDED return
`requiresAcceptance: false` and advance through internal shadow reservation and
execution without a manual quote/accept/reserve chain. Ordinary `/execute` also
handles that internal reservation. Quote-only remains inert. Ordinary `/accept`
is a no-op; acceptedAt stays null instead of fabricating explicit consent.

DEEP_ANALYSIS and RESEARCH_REPORT return `requiresAcceptance: true` and stop at
the quote. Their explicit POST `operations/:id/accept`, `/reserve`, `/execute`
workflow remains available. The R2 acceptance CHECK scopes consent requirements
to these classes; the R1 Sand charging hard-off CHECK is unchanged. Valid stored
reuse takes precedence over original deep/report intent and never executes the
adapter. Settlement stays internal; `/release` cancels outstanding work and
internal `refund()` records a zero-value REFUND. No frontend or paid UX is wired.

`validatePlan()` rejects contextual plans that also request deep/report execution,
including conflicts with explicit request intent. Large stored coverage counts
alone are not contradictory: they can describe structured contextual data. Pending
R1 plans are revalidated before fresh adapter execution; invalid plans release.
...
`20260922140000_ask_compute_sand_r1/migration.sql` and then
`20260922170000_ask_confirmation_boundary_r2/migration.sql`. Then run from repository root:
