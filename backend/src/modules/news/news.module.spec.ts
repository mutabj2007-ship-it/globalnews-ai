...
describe('NewsModule — real NestJS DI compilation and provider registration', () => {
  ...
  it('ALL_NEWS_PROVIDERS always contains EVERY registered provider, so an inactive provider stays visible to GET /news/providers/health', async () => {
    for (const apiKey of ['real-key', undefined]) {
      const moduleRef = await buildTestingModule(apiKey);

      const registered = moduleRef.get<NewsProvider[]>(ALL_NEWS_PROVIDERS);

      // R4 GDELT — three now: the mock wire, GNews and GDELT DOC. The count
      // is asserted rather than merely the memberships, because the point
      // of this token is that NOTHING is quietly dropped from health
      // reporting — a fourth provider appearing here unannounced should
      // fail this test.
      ...
      expect(registered.some((provider) => provider instanceof GdeltDocProvider)).toBe(true);
  ...
  /**
   * R4 GDELT — TEST M, at the wiring level.
   *
   * GDELT DOC is REGISTERED unconditionally but ACTIVE only when
   * GDELT_DOC_ENABLED is exactly "true". Both halves matter: registered so
   * Admin -> Global Sources can show it truthfully as switched off, and
   * inactive so it can never serve or be called.
   */
  it('GDELT DOC is registered but INACTIVE unless GDELT_DOC_ENABLED is exactly "true"', async () => {
    for (const flag of [undefined, '', '  ', 'false', '1', 'yes', 'TRUE ']) {
      if (flag === undefined) {
        delete process.env.GDELT_DOC_ENABLED;
      } else {
        process.env.GDELT_DOC_ENABLED = flag;
      }
      ...
      const active = moduleRef.get<NewsProvider[]>(NEWS_PROVIDERS);
      const registered = moduleRef.get<NewsProvider[]>(ALL_NEWS_PROVIDERS);
      const fallback = moduleRef.get<NewsProvider[]>(FALLBACK_NEWS_PROVIDERS);

      const enabled = flag?.trim().toLowerCase() === 'true';

      expect(registered.some((provider) => provider.id === 'gdelt-doc')).toBe(true);
      expect(active.some((provider) => provider.id === 'gdelt-doc')).toBe(enabled);
      expect(fallback.some((provider) => provider.id === 'gdelt-doc')).toBe(enabled);
  ...
  /**
   * R4 GDELT — the fallback token is a SUBSET of the active set, never a
   * disjoint one. If these ever diverged, a fallback provider could be
   * called while being absent from health reporting and from the
   * cross-provider dedup rank list.
   */
  it('FALLBACK_NEWS_PROVIDERS is always a subset of NEWS_PROVIDERS', async () => {
    process.env.GDELT_DOC_ENABLED = 'true';
    ...
    // GNews is primary, so it must NOT be in the fallback set.
    expect(fallback.some((provider) => provider instanceof GNewsProvider)).toBe(false);
  ...
  /**
   * R4 GDELT — with zero real providers configured the active set is
   * exactly [mock], and there is nothing to tier.
   */
  it('no real provider configured leaves the fallback tier empty and the mock answering alone', async () => {
    process.env.GDELT_DOC_ENABLED = 'false';
