...
describe('Milestone #63 — bounded domain-aware supplemental retrieval', () => {
  ...
  function narrowPrimaryArticles() {
    return [
      ...
      makeArticle({
        ...
        summary:
          'Drones are used to deliver blood and medical supplies to the community in rural Rwanda.',
  ...
  it('deterministic first-mentioned-domain priority: supplemental searches target political and economic (the first two missing domains in question order), not later-mentioned ones', async () => {
    ...
    const searchedTerms = (newsService.search as jest.Mock).mock.calls.map(
  ...
  describe('final-evidence reserved-slot selection (CTO correction)', () => {
    ...
    it('ordinary country retrieval (no supplements) preserves its previous articlesRetrieved semantics — the plain primary pool count', async () => {
      const primaryArticles = [
        makeArticle({ id: 'ordinary-1', title: 'Rwanda general news update' }),
      ];
      const newsService = { search: jest.fn() };
      const countryNewsService = {
        getCountryNews: jest
...
/* ------------------------------------------------------------------ *
 * GEO-PRECISION-1 — GEOGRAPHIC RESOLUTION REGRESSION MATRIX
 * ------------------------------------------------------------------ */

/**
 * The defect this matrix exists to prevent coming back:
 *
 *   "in Musanze, Rwanda"  ->  NOTHING. No country, no articles, no analysis.
 *   "in Rwanda"           ->  Rwanda, a full country-level answer.
 *
 * Naming the city DESTROYED the country. The candidate scan built PREFIXES
 * only, so the word "Rwanda" — unambiguous, curated, and sitting in plain
 * sight after the comma — was never passed to the resolver at all. The same
 * fault silently swallowed "Rome, Italy", "Kyiv, Ukraine", "New Delhi,
 * India" and "Addis Ababa, Ethiopia".
 *
 * "Kigali, Rwanda" appeared to work, but only because a LAST-RESORT TYPO
 * CORRECTOR scored "kigali," as one insertion away from "kigali". Correct
 * punctuation was being reported to the reader as a misspelling, and that
 * rescue was never available to a curated city under MIN_TARGET_LENGTH or
 * one containing a space.
 *
 * Every case below is driven through the real AnalysisService against the
 * real shared resolvers. The mock CountryNewsService mirrors the production
 * envelope's `...(city ? { city } : {})` so `retrievalContext.city` is the
 * value the real service would have produced.
 */
describe('GEO-PRECISION-1 — geographic resolution regression matrix', () => {
  ...
  function buildHarness() {
    const articles = [makeArticle({ id: 'geo-1', title: 'A headline' })];
    ...
    const countryNewsService = {
      getCountryNews: jest
        .fn()
        .mockImplementation(
          async (iso3: string, _category: unknown, _limit: number, city?: string) =>
            makeCountryResponse(iso3, COUNTRY_NAMES[iso3] ?? iso3, articles, city ? { city } : {}),
        ),
    };
    ...
    return { service, newsService, countryNewsService };
  }

  async function resolve(query: string) {
    const harness = buildHarness();
    const response = await harness.service.analyzeNews(query);

    return {
      response,
      retrieval: response.retrievalContext,
      countryCall: harness.countryNewsService.getCountryNews.mock.calls[0],
      countryCalled: harness.countryNewsService.getCountryNews.mock.calls.length > 0,
      searchCalled: harness.newsService.search.mock.calls.length > 0,
    };
  }

  /**
   * THE MANDATORY MATRIX. `typoProvenance` is the property that separates a
   * genuine correction from correct punctuation being treated as one: it is
   * `matchedFrom` on the retrieval context, which is what the geographic card
   * prints as "Resolved from ...".
   */
  const MATRIX: {
    query: string;
    iso3: string;
    city: string | undefined;
    typoProvenance: boolean;
  }[] = [
    { query: 'Kigali, Rwanda', iso3: 'RWA', city: 'kigali', typoProvenance: false },
    { query: 'Kigali', iso3: 'RWA', city: 'kigali', typoProvenance: false },
    { query: 'Kigalli, Rwanda', iso3: 'RWA', city: 'kigali', typoProvenance: true },
    { query: 'Musanze, Rwanda', iso3: 'RWA', city: undefined, typoProvenance: false },
    { query: 'Rome, Italy', iso3: 'ITA', city: 'rome', typoProvenance: false },
    { query: 'Kyiv, Ukraine', iso3: 'UKR', city: 'kyiv', typoProvenance: false },
    { query: 'New Delhi, India', iso3: 'IND', city: 'new delhi', typoProvenance: false },
    {
      query: 'Addis Ababa, Ethiopia',
      iso3: 'ETH',
      city: 'addis ababa',
      typoProvenance: false,
    },
    {
      query: 'Washington DC, United States',
      iso3: 'USA',
      city: 'washington dc',
      typoProvenance: false,
    },
  ...
  for (const row of MATRIX) {
    const question = `What are the latest developments in ${row.query}?`;

    it(`"${row.query}" resolves to ${row.iso3}${row.city ? ` / ${row.city}` : ' at COUNTRY precision'}`, async () => {
      const { retrieval, countryCall, searchCalled } = await resolve(question);

      // Country-aware retrieval ran. Generic keyword search did not.
      expect(countryCall?.[0]).toBe(row.iso3);
      expect(searchCalled).toBe(false);

      // The city handed to CountryNewsService is the CANONICAL curated key —
      // never a punctuated form. buildSearchTerm() and articleMentionsCity()
      // both consume this value directly.
      expect(countryCall?.[3]).toBe(row.city);

      expect(retrieval.countryCode).toBe(row.iso3);
      expect(retrieval.city).toBe(row.city);
    });

    it(`"${row.query}" ${row.typoProvenance ? 'discloses' : 'does NOT invoke'} typo provenance`, async () => {
      const { retrieval } = await resolve(question);

      if (row.typoProvenance) {
        expect(retrieval.matchedFrom).toBeDefined();
        expect(retrieval.canonicalLocation).toBe(row.city);
      } else {
        // Correct punctuation is not a misspelling and must never be
        // reported as one.
        expect(retrieval.matchedFrom).toBeUndefined();
        expect(retrieval.canonicalLocation).toBeUndefined();
        expect(retrieval.matchConfidence).toBeUndefined();
      }
    });
  }

  it('AN EXPLICIT COUNTRY SUFFIX SURVIVES AN UNKNOWN LOCALITY — the core defect', async () => {
    const withLocality = await resolve('What are the latest developments in Musanze, Rwanda?');
    const withoutLocality = await resolve('What are the latest developments in Rwanda?');

    // Both reach Rwanda. Supplying MORE correct geography must never return
    // strictly less than supplying less.
    expect(withLocality.countryCall?.[0]).toBe('RWA');
    expect(withoutLocality.countryCall?.[0]).toBe('RWA');
    expect(withLocality.searchCalled).toBe(false);

    // And the answer is country precision, not a fabricated city.
    expect(withLocality.retrieval.city).toBeUndefined();
    expect(withLocality.retrieval.countryName).toBe('Rwanda');
  });

  it('MUSANZE IS NOT INVENTED AS A CURATED CITY — it resolves through the country alone', async () => {
    const { retrieval, countryCall } = await resolve(
      'What are the latest developments in Musanze, Rwanda?',
    );

    // No city argument, no city in the context, and no fuzzy correction
    // quietly mapping Musanze onto some curated neighbour.
    expect(countryCall?.[3]).toBeUndefined();
    expect(retrieval.city).toBeUndefined();
    expect(retrieval.matchedFrom).toBeUndefined();
    expect(retrieval.canonicalLocation).toBeUndefined();
  });

  it('a bare uncurated locality with NO country still resolves to nothing, and is not guessed', async () => {
    const { countryCalled, searchCalled } = await resolve(
      'What are the latest developments in Musanze?',
    );

    // Nothing in this repair may manufacture geography from a place the
    // system genuinely does not know.
    expect(countryCalled).toBe(false);
    expect(searchCalled).toBe(true);
  });

  it('GENUINE TYPO CORRECTION STILL WORKS, with and without a country suffix', async () => {
    const bare = await resolve('What is happening in Kigalli?');
    expect(bare.countryCall?.[0]).toBe('RWA');
    expect(bare.countryCall?.[3]).toBe('kigali');
    expect(bare.retrieval.matchedFrom).toBe('kigalli');

    const suffixed = await resolve('What is happening in Kigalli, Rwanda?');
    expect(suffixed.countryCall?.[0]).toBe('RWA');
    expect(suffixed.countryCall?.[3]).toBe('kigali');
    expect(suffixed.retrieval.matchedFrom).toBe('kigalli');
    expect(suffixed.retrieval.matchConfidence).toBeGreaterThanOrEqual(80);
  });

  it('a fuzzy city correction is refused when it does not belong to the explicit country', async () => {
    // "Warsav" corrects to Warsaw, which is Poland — not Rwanda. The
    // explicitly named country must win rather than the answer being dragged
    // to whichever country the typo happened to land in.
    const { countryCall, retrieval } = await resolve('What is happening in Warsav, Rwanda?');

    expect(countryCall?.[0]).toBe('RWA');
    expect(countryCall?.[3]).toBeUndefined();
    expect(retrieval.city).toBeUndefined();
  });

  it('multi-word single-segment queries are untouched by segmentation', async () => {
    const { countryCall } = await resolve('What is happening in the United States today?');

    expect(countryCall?.[0]).toBe('USA');
    expect(countryCall?.[3]).toBeUndefined();
  });

  it('ZERO-EVIDENCE PROTECTION REMAINS INTACT on the repaired country path', async () => {
    // Rwanda resolves, retrieval returns nothing, and the response must still
    // refuse to claim an analysis it never made.
    const countryNewsService = {
      getCountryNews: jest
        .fn()
        .mockResolvedValue(makeCountryResponse('RWA', 'Rwanda', [], { city: undefined })),
    };
    const newsService = { search: jest.fn() };
    const provider: AnalysisProvider = {
      id: 'mock-analysis',
  ...
  it('GEOGRAPHIC PRECISION IS A RETRIEVAL FACT — the context never claims city-level EVIDENCE', async () => {
    const { retrieval } = await resolve('What are the latest developments in Kigali, Rwanda?');

    // `city` is the city the QUERY resolved to, echoed back. There is no
    // per-article geographic field on the contract, and none is invented
    // here. Nothing in the retrieval context asserts anything about where
    // the returned articles are actually about.
    expect(retrieval.city).toBe('kigali');
    expect(Object.keys(retrieval)).not.toContain('cityEvidence');
    expect(Object.keys(retrieval)).not.toContain('geographicPrecision');
    expect(Object.keys(retrieval)).not.toContain('articlesMatchingCity');
  });
});
