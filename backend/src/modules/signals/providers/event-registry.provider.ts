...
export class EventRegistryProvider implements SignalProvider {
  ...
  private async request(url: string): Promise<EventRegistryEventsEnvelope> {
    this.requestCount += 1;
    const timeoutMs =
      this.config.get<number>('EVENT_REGISTRY_REQUEST_TIMEOUT_MS') || DEFAULT_TIMEOUT_MS;
    ...
    if (response.status === 429) {
      ...
      this.lastRateLimitState = 'throttled';
      throw new EventRegistryProviderError(
        'Event Registry rate limit exceeded. Try again shortly.',
        undefined,
        true,
      );
    }
    if (!response.ok) {
      this.failureCount += 1;
      throw new EventRegistryProviderError(
        `Event Registry responded with status ${response.status}.`,
      );
    }
    ...
    try {
      ...
      this.failureCount += 1;
      throw new EventRegistryProviderError(
        'Event Registry returned a malformed (non-JSON) response.',
        error,
      );
    }
    ...
    if (
      ...
      this.failureCount += 1;
      throw new EventRegistryProviderError(
        'Event Registry response did not match the expected shape.',
      );
    }
  ...
  private toGeoSignal(
    ...
    const toneScore =
      typeof event.sentiment === 'number' && Number.isFinite(event.sentiment)
        ? event.sentiment
        : undefined;
