...
/*
  The destination derives its source set and relational evidence from THE SAME
  builders the frame uses, against the same `response`. Passing the frame's
  already-built model down would have meant lifting the frame's state up for
  one destination; calling the same pure builders keeps both surfaces reading
  one definition of what a source is, which is the property that matters.
*/
...
type Destination = 'frame' | 'record' | 'library' | 'sources';
...
export function AnalysisFrameClient({
  ...
  /*
    PO C904 REVIEW, CORRECTION 2 — THE DEDICATED SOURCES DESTINATION.

    The fourth member of an existing set, wired identically to the other three.
    No route, no navigation, no refetch: `response` is held here, so replacing
    the frame with this page and coming back is a state change and the reader's
    question and analysis survive it untouched.

    The frame opens this ONLY when it has measured that the expanded dock
    cannot coexist with a useful reader. Everywhere else the dock expands in
    place, which is the behaviour the ruling prefers.
  */
