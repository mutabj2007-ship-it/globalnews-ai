...
export function resolveTracks(input: TrackInput): FrameTracks {
  ...
  // The largest dock that still leaves the centre its floor. The DOCK is
  // what gets clamped — never the centre, never the brief, and never the
  // frame, which is why no other track's offset can move (F-6).
  const dockCeiling = frameHeight - briefHeight - CENTRE_MIN_HEIGHT;
  ...
  if (dock === 'compact') {
    ...
    dockHeight = Math.max(compactDock, Math.min(desired, dockCeiling));
  }
