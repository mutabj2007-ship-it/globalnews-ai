...
export interface BriefModel {
  ...
  /**
   * C907 CORRECTION 3 — true when the backend WITHHELD the executive brief
   * because it was measured non-compliant and the one permitted repair failed
   * too. `paragraph` is the empty string on that path, by construction: the
   * backend does not transmit the rejected prose at all.
   *
   * A surface must branch on this rather than on `paragraph === ''`, because
   * those two facts are not the same. An empty paragraph with
   * `briefWithheld: false` means the analysis simply carried no summary — an
   * absence nobody measured. `briefWithheld: true` means the product ASSESSED
   * a brief and refused it, and the reader is owed that difference in words.
   */
  readonly briefWithheld: boolean;
  /**
   * C907 — the backend's plain-English statement of the structural defect,
   * present only when `briefWithheld`. Carried verbatim; never re-worded here,
   * and never shown as if it were the brief.
   */
  readonly briefWithheldReason: string | null;
  /**
...
export function buildBriefModel(response: AnalysisApiResponse): BriefModel {
  ...
  return {
    ...
    /*
      C907 — read from the contract field, never inferred from the empty
      string. `briefState` is optional for backward compatibility and its
      absence means "not assessed", which is NOT withheld.
    */
    briefWithheld: analysis.briefState?.availability === 'withheld-non-compliant',
    briefWithheldReason: analysis.briefState?.reason ?? null,
