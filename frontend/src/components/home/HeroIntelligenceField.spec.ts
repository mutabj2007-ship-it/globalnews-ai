  ...
  */
  it('takes a resolved focus target and nothing else — it still exposes no interaction and decides nothing', () => {
    ...
    */
    ...
    /*
      CONVERTED UNDER CTO STEP 1 (decision F-1).

      The focus prop previously carried a resolved point AND a colour channel.
      F-1 replaced the per-category channel with ONE fixed amber locator, so the
      channel field is gone and the prop is now a bare resolved point. The
      assertion is converted rather than deleted: it still checks the same
      declaration, and it now proves the channel is ABSENT — the field may not
      re-acquire a category vocabulary of its own.

      Everything L-8 refused is still refused: a name, label, id or tooltip here
      would be the named, described signal node CTO decision L-8 rejected, and
      this field is aria-hidden so nothing in it may carry an accessible name.
    */
    ...
    expect(focusBlock).not.toMatch(/channel/);
    // The point is the WHOLE payload — two fields, nothing else.
    expect((focusBlock.match(/^\s*\w+: [^;]+;/gm) ?? [])).toHaveLength(2);
