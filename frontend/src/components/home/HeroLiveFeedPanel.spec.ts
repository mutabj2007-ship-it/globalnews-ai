...
/*
  ...
  The panel had ONE empty branch, reached by `rows.length === 0`, and it says
  "Live feed temporarily unavailable." Combined with the allocator subtracting
  the twelve editorial roles out of the stream, a perfectly healthy live day
  reached that branch and announced a source failure.

  These tests are source-level, like every other contract in this file: the
  panel is a server-rendered presentation with no test renderer in this
  repository, so the released convention is to assert the construction. What
  they pin is that the empty branch is decided by the AUTHORITATIVE provenance
  value and not by a count.
*/
describe('C907 — the three empty states', () => {
  it('resolves the empty state from statusKey, never from how many rows came back', () => {
    expect(code).toMatch(/const emptyState: 'no-current-stories' \| 'demo' \| 'unavailable' =/);
    expect(code).toMatch(/statusKey === 'live' \|\| statusKey === 'cached'/);
    expect(code).toMatch(/statusKey === 'mock'/);

    /* The decision must not read a count anywhere in it. */
    const decision = code.slice(
      code.indexOf('const emptyState:'),
      code.indexOf("      : 'unavailable';") + 20,
    );
    expect(decision).not.toMatch(/rows|articles|length|hasArticles/);
  });

  it('covers every LiveStatusKey member, so no state can fall through', () => {
    const liveStatus = readFileSync(
      join(__dirname, '..', '..', 'lib', 'liveStatus.ts'),
      'utf-8',
    );
    const union = liveStatus.slice(
      liveStatus.indexOf('export type LiveStatusKey'),
      liveStatus.indexOf("  | 'unknown';") + 14,
    );
    const members = [...union.matchAll(/'([a-z-]+)'/g)].map((m) => m[1]);

    expect(new Set(members)).toEqual(
      new Set(['live', 'cached', 'mock', 'unavailable', 'reconnecting', 'unknown']),
    );

    /*
      Two members are named explicitly and the remaining four fall to the
      failure branch. That is exhaustive by construction — the ternary has no
      unreachable arm and no default that silently absorbs a NEW member,
      because adding one to the union would land it in `unavailable`, which is
      the conservative direction: a state nobody has classified is reported as
      "we could not retrieve", never as "everything is fine".
    */
    expect(code).toMatch(/\? 'no-current-stories'/);
    expect(code).toMatch(/\? 'demo'/);
    expect(code).toMatch(/: 'unavailable';/);
  });

  it('gives the successful-but-empty state its own words, which claim no fault', () => {
    expect(source).toContain('t.feedPanelEmptyHeading');
    expect(source).toContain('t.feedPanelEmptyBody');
    for (const dictionary of [enDictionary, plDictionary]) {
      expect(dictionary).toContain('feedPanelEmptyHeading');
      expect(dictionary).toContain('feedPanelEmptyBody');
    }
    /* The English text states the retrieval succeeded rather than hedging. */
    expect(enDictionary).toMatch(/feedPanelEmptyBody: 'Retrieval succeeded\./);
  });

  it('gives sample content its own words too, instead of borrowing the failure text', () => {
    expect(source).toContain('t.feedPanelDemoHeading');
    expect(source).toContain('t.feedPanelDemoBody');
    for (const dictionary of [enDictionary, plDictionary]) {
      expect(dictionary).toContain('feedPanelDemoHeading');
      expect(dictionary).toContain('feedPanelDemoBody');
    }
  });

  it('reaches the RELEASED unavailable presentation only through the failure state', () => {
    /*
      The released block is unchanged — heading, body, the subsystem status
      list and the footer are all still there — but it is now inside the
      `emptyState === 'unavailable'` arm rather than being the whole branch.
    */
    const unavailableArm = source.slice(
      source.indexOf("{emptyState === 'unavailable' ? ("),
      source.indexOf('THE TWO NON-FAULT STATES'),
    );

    expect(unavailableArm).toContain('{t.feedPanelUnavailableHeading}');
    expect(unavailableArm).toContain('{t.feedPanelUnavailableBody}');
    expect(unavailableArm).toContain('{t.feedPanelUnavailableFooter}');
    expect(unavailableArm).toMatch(
      /\[t\.feedPanelSearchStatus, t\.feedPanelCountryStatus, t\.feedPanelMapStatus\]\.map/,
    );
    /* The non-fault states must NOT re-assert subsystem availability. */
    const nonFaultArm = source.slice(source.indexOf('THE TWO NON-FAULT STATES'));
    expect(nonFaultArm).not.toContain('feedPanelSearchStatus');
    expect(nonFaultArm).not.toContain('feedPanelAvailable');
  });

  it('still fabricates no headline and no timestamp in ANY empty state', () => {
    const allEmptyStates = source.slice(
      source.indexOf('{emptyState ==='),
      source.indexOf('feedPanelViewMap'),
    );

    expect(allEmptyStates).not.toMatch(/formatRelativeTime/);
    expect(allEmptyStates).not.toMatch(/item\./);
    expect(allEmptyStates).not.toMatch(/Date|toLocale/);
  });

  it('the panel itself is still present in every empty state — the surface never disappears', () => {
    expect(source).toMatch(/hasArticles \? \(/);
    expect(code).not.toMatch(/hasArticles && \(/);
  });
});
