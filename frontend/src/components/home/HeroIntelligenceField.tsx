 ...
 */
/**
 * STEP 1 (CTO decision F-1) — THE SELECTED-INTELLIGENCE LOCATOR COLOUR.
 *
 * Cyan/blue is ambient intelligence; yellow/amber is the CURRENTLY SELECTED
 * intelligence. The ring stack previously took the focused article's CATEGORY
 * channel, which meant a selected signal could render in the same cyan as the
 * ambient field and read as one more decorative mark. A fixed focal colour
 * makes selection legible as selection.
 *
 * This is the released `cd.amber` (#fbbf24) from tailwind.config.ts, the same
 * value the Live Feed's own amber family already uses — not a new colour, and
 * not a second category table. The field states colours as literals (see
 * DECORATIVE_MARKS above), so this belongs here rather than being threaded in
 * as a prop: the caller now supplies a PLACE, and the field owns how selection
 * looks.
 *
 * WHAT THIS DOES NOT DO. It is applied only when a real focus with resolved
 * geography arrives. With no focus — including a focused article whose country
 * did not resolve — the emphasis stays on the released decorative mark in its
 * released colour, so no amber locator is ever drawn for a story whose location
 * is unknown. That is CTO decision F-1's "unresolved geography produces no map
 * locator", enforced by the same null check that already governed the position.
 */
const FOCUS_LOCATOR_COLOR = '#fbbf24';

/**
 * G2 — THE RETICLE'S GEOMETRY, IN VIEWBOX UNITS.
 *
 * DECORATIVE_RING_BASE is 14, so the inner emphasis ring sits at r=14 and the
 * decorative nodes are r<=8 halos. A gap at 11 with ticks reaching 20 therefore
 * BRACKETS the selected point at country scale without covering it, and lands
 * outside every ambient mark's own radius. Four ticks, on the two axes: a
 * shape nothing else in this field draws.
 */
const RETICLE_GAP = 11;
const RETICLE_REACH = 20;
const RETICLE_TICKS: Array<[number, number]> = [
  [0, -1],
  [0, 1],
  [-1, 0],
  [1, 0],
...
export function HeroIntelligenceField({
  ...
  /*
    ...
    today's hero. When a feed row or a Global Developments card is focused, the
    SAME ring stack — same DECORATIVE_RING_BASE, same multipliers, same
    per-ring strokes, same animation — relocates to the focused country. No
    ring is added, no geometry is redrawn, and all ten decorative marks and
    eight links are untouched.

    STEP 1 / CTO DECISION F-1 SUPERSEDES ONE SENTENCE HERE. This comment used
    to say the relocated stack takes the article's CATEGORY colour. It no
    longer does: the innermost ring and the core take FOCUS_LOCATOR_COLOR, one
    fixed amber, because the colour now means "this is the current selection"
    rather than "this is a health story". The outer rings stay the released
    cyan, so ambient intelligence and selected intelligence stay distinguishable.
  ...
  */
  ...
  const focusColor = focus ? FOCUS_LOCATOR_COLOR : FOCUS_MARK.color;
    ...
    <div aria-hidden="true" className="relative h-full w-full overflow-hidden">
      ...
      >
        ...
        <g fill="rgba(13,48,88,0.62)" stroke="rgba(56,189,248,0.42)" strokeWidth="0.55">
          {/* The numeric id is not unique across this collection: several features
             carry "000", so React saw repeated keys and warned that children may be
             "duplicated and/or omitted". The map position disambiguates them without
             inventing an identifier and without touching the geometry. */}
          {collection.features.map((feature: CountryFeature, index: number) => (
            <path key={`${feature.properties.numericId}-${index}`} d={countryPathD(feature.geometry as Geometry, keepEvery)} />
          ))}
        </g>
        ...
        <g>
          ...
          {/*
            G2 — THE SELECTED-LOCATION TARGET (RETICLE).

            WHY A RETICLE AND NOT A BRIGHTER DOT. Colour alone could not carry
            this. DECORATIVE_MARKS above already contains THREE #fb923c marks —
            and FOCUS_MARK, the idle emphasis, is one of them. Against those the
            selected amber shares its entire red channel and sits about 17 deg
            away in hue: two saturated oranges, ~7px across, on a dark map. The
            selection has to differ in FORM, and no ambient mark has ticks.

            ONLY WHEN GEOGRAPHY RESOLVED. Rendered under `focus &&`, and `focus`
            is non-null only when the caller resolved a real country to a real
            centroid. An article whose country did not resolve produces no
            reticle at all — the map keeps its released idle emphasis and claims
            nothing. Focus changing to another country moves the whole group,
            because every coordinate below derives from focusPoint.

            IT CLAIMS NO PRECISION IT DOES NOT HAVE. The ticks stand OFF the
            point, opening a gap the size of the country ring rather than
            spiking a pin at one pixel. That matches the evidence exactly:
            countryFocusPoint() is a bounding-box centre for a COUNTRY, and the
            card beside it says COUNTRY-LEVEL EVIDENCE in words.

            Still no id, no label, no tabindex, no tooltip: this field is
            aria-hidden, and the card carries the meaning in text (CTO L-8).
          */}
          {focus ? (
            <g>
              {RETICLE_TICKS.map(([dx, dy], index) => (
                <line
                  key={index}
                  x1={focusPoint[0] + dx * RETICLE_GAP}
                  y1={focusPoint[1] + dy * RETICLE_GAP}
                  x2={focusPoint[0] + dx * RETICLE_REACH}
                  y2={focusPoint[1] + dy * RETICLE_REACH}
                  stroke={FOCUS_LOCATOR_COLOR}
                  strokeWidth="1.6"
                  strokeLinecap="round"
                />
              ))}
