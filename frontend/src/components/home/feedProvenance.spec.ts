...
describe('M66.13 — the feed panel derives liveness from the authoritative model', () => {
  ...
  it('the status heading is state-resolved and covers every state the resolver can return', () => {
    ...
    /*
      CONVERTED UNDER CTO-APPROVED STEP 5A, AND DELIBERATELY STRICTER THAN
      WHAT IT REPLACED.

      This used to assert the literal `{statusHeading}`. Step 5A adds a
      country-focus qualifier to the same amber line, so the rendered
      expression is now a two-branch ternary and that literal no longer
      appears. Nothing this test was protecting has changed: statusHeading is
      still the ONE place dataMode becomes user-facing text, it is still
      computed from all five resolver states above, and it is still what
      renders whenever no country is focused.

      The replacement names that property directly instead of matching a
      substring, so it now fails for a REASON - a country qualifier that
      displaced the provenance heading in the global state - rather than
      failing for a rename. The Step 5A gate that keeps DEMO/CACHED out of the
      qualifier entirely lives in HeroLiveFeedPanel.spec.ts and is asserted
      there as well.
    */
    expect(panelCode).toMatch(/focusHeading === null \? statusHeading : focusHeading/);
  });
