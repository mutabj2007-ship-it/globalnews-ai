...
describe('GlobalDevelopments (M60 Phase 2 — controlled carousel)', () => {
  it('reuses Phase B semantic allocation (featured/inFocus/discovery) — no second allocation truth', () => {
    expect(source).toMatch(/lead: NewsArticle \| null/);
    expect(source).toMatch(/secondary: NewsArticle\[\]/);
    expect(source).toMatch(/discovery: NewsArticle\[\]/);
    expect(stripComments(source)).not.toMatch(/\.sort\(/);
    expect(stripComments(cardSource)).not.toMatch(/\.sort\(/);
  });

  /*
    CONVERTED UNDER CTO-APPROVED STEP 4 — ROLE SEPARATION.

    This capped the rail at lead + 5, which made Global Developments a strict
    SUBSET of the Live Feed's top rows: the same six stories, restated as cards.
    That is the duplication Step 4 exists to remove.

    The cap becomes the full CURATED collection — 1 featured + 5 inFocus + 6
    discovery — matching allocateHomeFeed's own defaults. `feed.discovery` was
    already fetched, allocated and typed on every page load and rendered by
    nothing; consuming it is the whole of the change.

    IT IS A COLLECTION CAP, NOT A VIEWPORT. Nothing about the rail's geometry
    moves — cards beyond the viewport go into the horizontal overflow that
    already exists, per CTO decision 'available is not simultaneously visible'.
  */
  it('STEP 4 — the rail carries the full curated collection: lead + secondary + discovery', () => {
    expect(source).toMatch(/const CURATED_CARD_COUNT = 12/);
    expect(source).toMatch(
      /\[lead, \.\.\.secondary, \.\.\.discovery\]\.slice\(0, CURATED_CARD_COUNT\)/,
    );

    // The retired six-card cap must not come back by either name.
    expect(stripComments(source)).not.toMatch(/SECONDARY_COUNT/);
  });

  it('STEP 4 — the rail geometry is UNCHANGED; extra cards go to horizontal overflow', () => {
    /*
      The one thing that could betray this change: enlarging the surface to show
      everything at once. The rail must still be a horizontal scroller with its
      measured-overflow arrows, and must not wrap into rows or grow vertically.
    */
    expect(source).toMatch(/overflow-x-auto/);
    expect(stripComments(source)).not.toMatch(/flex-wrap[^"]*" *>[\s\S]{0,80}TrendingCard/);
    expect(source).toMatch(/scrollWidth > el\.clientWidth/);
  });

  it('STEP 4 — Global Developments introduces NO retrieval of its own', () => {
    expect(stripComments(source)).not.toMatch(/\bfetch\(|axios|useSWR|useQuery|\/api\//);
    expect(stripComments(cardSource)).not.toMatch(/\bfetch\(|axios|useSWR|useQuery|\/api\//);
  });
