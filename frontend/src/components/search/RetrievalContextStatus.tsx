...
export function RetrievalContextStatus({
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      <span
        className={`inline-flex w-fit items-center gap-1.5 rounded-full border px-2.5 py-1 font-mono text-[12px] md:text-[11px] font-semibold uppercase tracking-wide ${BADGE_STYLES[dataMode]}`}
      >
      ...
      {freshnessLine && (
        <p className="font-mono text-[12px] md:text-[11px] text-ink-tertiary">{freshnessLine}</p>
      )}
    </div>
  );
}
