    ...
    <span className="ml-2 inline-flex flex-wrap items-center gap-1 align-middle">
      {chips.map((chip) => (
        <a
          ...
          title={chip.title}
          className="inline-flex h-4 min-w-4 items-center justify-center rounded-full border border-border-strong bg-surface px-1 font-mono text-[12px] md:text-[11px] leading-none text-ink-tertiary transition-colors hover:border-signal/60 hover:text-signal-bright"
        >
          {chip.number}
        </a>
      ))}
