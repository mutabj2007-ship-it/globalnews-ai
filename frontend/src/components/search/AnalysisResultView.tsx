    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="rounded-2xl border border-border bg-surface p-6 sm:p-8">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="font-mono text-[12px] md:text-[11px] text-ink-tertiary">
        ...
        {/*
          ══════════════════════════════════════════════════════════════════
          C907 CORRECTION 3 — A WITHHELD BRIEF IS STATED, NOT LEFT BLANK
          ══════════════════════════════════════════════════════════════════

          `analysis.summary` is the empty string whenever the backend refused
          the brief, because the rejected prose is not transmitted at all.
          Rendering it unconditionally would leave an empty paragraph under
          the headline — an absence the reader cannot distinguish from a
          rendering fault. The contract field is what is branched on, never
          the emptiness of the string: an analysis that merely never carried a
          summary was not assessed and is not the same event.
        */}
        {analysis.briefState?.availability === 'withheld-non-compliant' ? (
          <div className="rounded-xl border border-border bg-page/40 p-4">
            <p className="font-mono text-[12px] uppercase tracking-wide text-ink-tertiary">
              {t.briefWithheldHeading}
            </p>
            <p className="mt-2 text-sm leading-relaxed text-ink-secondary">
              {t.briefWithheldBody}
            </p>
            {analysis.briefState.reason !== undefined && (
              <p className="mt-2 text-[13px] leading-relaxed text-ink-tertiary">
                {analysis.briefState.reason}
              </p>
            )}
          </div>
        ) : (
      ...
      */}
      {analysis.relationalComposition && (
        <div className="rounded-2xl border border-border bg-surface p-5">
          ...
          <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-1 font-mono text-[12px] md:text-[11px] uppercase tracking-wide text-ink-tertiary sm:grid-cols-4">
            <dt>{t.supporting}</dt>
      ...
      */}
      <details className="rounded-2xl border border-border-strong bg-surface p-4">
        <summary className="cursor-pointer font-mono text-[12px] md:text-[11px] uppercase tracking-wide text-ink-tertiary">
          {t.aiSelfAssessment}
        </summary>
        <div className="mt-3">
          <p className="mb-1 font-mono text-[12px] md:text-[11px] uppercase tracking-wide text-ink-tertiary">
          ...
          <p className="mt-2 text-[12px] md:text-[11px] italic leading-relaxed text-ink-tertiary">
      ...
      */}
      {analysis.significance && (
        <section>
          ...
          <span className="mb-3 inline-flex items-center rounded-full border border-border-strong px-2.5 py-1 font-mono text-[12px] md:text-[11px] uppercase tracking-wide text-ink-secondary">
            {
              {
            ...
            {analysis.affectedParties.map((item, index) => (
              ...
              >
                <span className="mb-1 flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full border border-border-strong px-2 py-0.5 font-mono text-[12px] md:text-[11px] uppercase tracking-wide text-ink-tertiary">
                    {item.partyType}
                  </span>
                </span>
                {item.effect}
      ...
      {entityGroups.length > 0 && (
        <section>
          <SectionHeading>
            ...
            */}
            <span className="ml-2 font-mono text-[12px] md:text-[11px] normal-case tracking-normal text-ink-tertiary">
          ...
          <div className="flex flex-col gap-3">
            {entityGroups.map((group) => (
              <div key={group.label} className="flex flex-wrap items-center gap-2">
                <span className="font-mono text-[12px] md:text-[11px] uppercase tracking-wide text-ink-tertiary">
                  {group.label}:
                </span>
