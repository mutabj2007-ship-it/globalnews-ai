...
export function AdminAccessState({
  ...
  /*
    M-ALPHA-AUTH — return the administrator to the admin page they were actually
    on, not the admin root. The backend allowlist accepts /admin and its
    lowercase sub-segments, so the admin sub-pages round-trip.

    The fallback is ADMIN_ROUTES.overview rather than a literal, because
    adminRoutes.ts is the ONE place an admin path string may be written in this
    codebase and adminRouteManifest.spec.ts enforces it. That guard caught this
    line when it was first written as a literal; the guard was right and the
    line was changed, rather than the other way round.

    RETURNING TO /admin IS NOT BEING ADMITTED TO IT. AdminGuard runs on every
    admin request regardless of how the browser arrived, so an ordinary user who
    signs in from this screen is returned here and shown exactly the same
    refusal. This link decides where the browser lands; RBAC decides what it
    finds.
  */
  const adminReturnDestination = usePathname() ?? ADMIN_ROUTES.overview;
    ...
    >
      <div className="w-full max-w-md rounded-xl border border-adm-edge bg-adm-card-soft p-7">
        ...
        {variant === 'unauthenticated' && (
          <a
            href={accountSignInUrl(adminReturnDestination)}
