...
/**
 * H2B typography reconciliation — layout.tsx's font declarations, parsed.
 *
 * The assertions below used to count occurrences of `preload: false` and
 * require the total to be 3. A count is a fact about how many faces happened
 * to exist on the day it was written: it breaks the moment an approved design
 * adds one, and it never said WHICH face carried the flag — the thing M66.1
 * actually cared about. These read the declarations instead, so the contract
 * is per-face and survives an approved addition.
 */
interface FontDeclaration {
  readonly name: string;
  readonly family: string;
  readonly variable: string;
  readonly preloadDisabled: boolean;
}

function fontDeclarations(source: string): FontDeclaration[] {
  const code = stripComments(source);
  const out: FontDeclaration[] = [];
  const pattern = /const (\w+) = (\w+)\(\{([\s\S]*?)\}\);/g;
  let match: RegExpExecArray | null = pattern.exec(code);
  while (match !== null) {
    const body = match[3] ?? '';
    const variable = /variable:\s*'([^']+)'/.exec(body)?.[1];
    /* Only next/font calls bind a CSS variable; everything else is skipped. */
    if (variable !== undefined) {
      out.push({
        name: match[1] ?? '',
        family: match[2] ?? '',
        variable,
        preloadDisabled: /preload:\s*false/.test(body),
      });
    }
...
function declarationsByVariable(source: string): Map<string, FontDeclaration> {
  return new Map(fontDeclarations(source).map((d) => [d.variable, d]));
}

/**
 * The three faces every route pays for. /search, /map, /history, /workspace,
 * /privacy and /terms render entirely on these, so they must stay preloaded —
 * that is the direction of this contract that actually costs users something.
 */
const APPLICATION_WIDE_FONT_VARIABLES = ['--font-display', '--font-body', '--font-mono'];

/**
 * The 79 fontSize keys GN-CD-301 released, frozen by name.
 *
 * The assertion this replaces required EVERY fontSize key to start with `cd-`,
 * which was true when `cd-*` was the only namespace and became false the
 * moment the approved Analysis Workspace scale landed beside it. Freezing the
 * released surface by name protects it far more precisely than a prefix test:
 * a removal fails, a rename fails, and a quiet addition to the cd-* scale
 * fails — while an approved sibling namespace is free to coexist.
 */
const RELEASED_CD_FONT_SIZE_KEYS = [
  'cd-hero', 'cd-article', 'cd-hero-m', 'cd-screen-title', 'cd-brief-title', 'cd-menu-row',
  'cd-wordmark', 'cd-wordmark-m', 'cd-lockup', 'cd-preview-head', 'cd-card-head-m',
  'cd-preview-head-m', 'cd-lockup-m', 'cd-hero-copy', 'cd-summary', 'cd-input', 'cd-keyfacts',
  'cd-row-head', 'cd-card-head', 'cd-hero-copy-m', 'cd-action', 'cd-trust-body', 'cd-bullet',
  'cd-module-desc', 'cd-preview-summary', 'cd-footer-desc', 'cd-engine-sub-m', 'cd-nav-item',
  'cd-signin', 'cd-mono-eyebrow', 'cd-mono-feed', 'cd-mono-section', 'cd-mono-glyph',
  'cd-mono-panel', 'cd-mono-trust', 'cd-mono-status', 'cd-mono-nav', 'cd-mono-module',
  'cd-mono-code', 'cd-mono-category', 'cd-mono-chip', 'cd-mono-meta', 'cd-mono-readout',
  'cd-mono-lang-m', 'cd-mono-module-m', 'cd-mono-preview-cat', 'cd-mono-inspect',
  'cd-mono-meta-m', 'cd-mono-preview-cat-m', 'cd-mono-chip-m', 'cd-mono-badge', 'cd-mono-badge-m',
  'cd-mono-core-sub', 'cd-mono-subject-m', 'cd-mono-category-m', 'cd-mono-bullet-m',
  'cd-mono-subject', 'cd-feed-region', 'cd-feed-time', 'cd-mono-feed-action', 'cd-mono-expand',
  'cd-mono-section-m', 'cd-mono-eyebrow-m', 'cd-mono-tile', 'cd-engine-sub', 'cd-core-title',
  'cd-core-title-m', 'cd-mono-trust-m', 'cd-hiw-title', 'cd-hiw-step-title', 'cd-mono-step',
  'cd-trust-subhead', 'cd-trust-body-m', 'cd-mono-glyph-m', 'cd-footer-ident-m',
  'cd-mono-tagline-m', 'cd-footer-legal-m', 'cd-footer-link', 'cd-mono-plate',
] as const;

/**
 * Namespaces allowed in the type scale. `cd-*` is GN-CD-301; `gn-*` is the
 * Analysis Workspace scale from GN-CD-ANALYSIS-WORKSPACE 01 §3, added
 * alongside rather than over it. A third prefix is namespace drift and fails.
 */
const APPROVED_FONT_SIZE_PREFIXES = ['cd-', 'gn-'];
...
describe('M66.1 — GN-CD-301 typography capability', () => {
  ...
  it('application-wide faces stay preloaded; every route-scoped face opts out', () => {
    const declarations = fontDeclarations(layoutSource);
    const applicationWide = declarations.filter((d) =>
      APPLICATION_WIDE_FONT_VARIABLES.includes(d.variable),
    );
    const routeScoped = declarations.filter(
      (d) => !APPLICATION_WIDE_FONT_VARIABLES.includes(d.variable),
    );

    /* Neither branch below may pass vacuously. */
    expect(applicationWide).toHaveLength(APPLICATION_WIDE_FONT_VARIABLES.length);
    expect(routeScoped.length).toBeGreaterThanOrEqual(3);

    for (const declaration of applicationWide) {
      expect(declaration.preloadDisabled).toBe(false);
    }
    for (const declaration of routeScoped) {
      expect(declaration.preloadDisabled).toBe(true);
    }
  });

  it('the three Claude Design faces are the ones M66.1 released, and stay unpreloaded', () => {
    const byVariable = declarationsByVariable(layoutSource);
    for (const [variable, family] of [
      ['--font-cd-display', 'Space_Grotesk'],
      ['--font-cd-body', 'IBM_Plex_Sans'],
      ['--font-cd-mono', 'IBM_Plex_Mono'],
    ] as const) {
      expect(byVariable.get(variable)?.family).toBe(family);
      expect(byVariable.get(variable)?.preloadDisabled).toBe(true);
    }
  });

  it('the Analysis Workspace faces are isolated additions on their own variables', () => {
    /*
      GN-CD-ANALYSIS-WORKSPACE 01 §3: "Two families only. Outfit — all prose,
      titles, entity names, values. JetBrains Mono — all HUD labels, counters,
      metadata, states, legends." Approved for that workspace, and for nothing
      else. Isolation is the whole contract: binding them to a variable an
      existing route already renders on is the mistake M66.1 documented when a
      widened family silently restyled seven files.
    */
    const byVariable = declarationsByVariable(layoutSource);
    expect(byVariable.get('--font-gn-display')?.family).toBe('Outfit');
    expect(byVariable.get('--font-gn-mono')?.family).toBe('JetBrains_Mono');
    expect(byVariable.get('--font-gn-display')?.preloadDisabled).toBe(true);
    expect(byVariable.get('--font-gn-mono')?.preloadDisabled).toBe(true);

    /* The legacy and Claude Design mono variables were not repointed. */
    expect(byVariable.get('--font-mono')?.family).toBe('IBM_Plex_Mono');
    expect(byVariable.get('--font-cd-mono')?.family).toBe('IBM_Plex_Mono');
  });

  it('the released cd-* type scale is intact, and an approved namespace may coexist (§I.2–I.4)', () => {
    const sizes = Object.keys(theme.fontSize);
    const cdSizes = sizes.filter((key) => key.startsWith('cd-'));

    /* 1. The released surface, exactly: nothing removed, nothing renamed, and
          nothing quietly added to the cd-* scale either. */
    expect([...cdSizes].sort()).toEqual([...RELEASED_CD_FONT_SIZE_KEYS].sort());
    expect(cdSizes).toHaveLength(RELEASED_CD_FONT_SIZE_KEYS.length);
    for (const key of RELEASED_CD_FONT_SIZE_KEYS) {
      expect(theme.fontSize[key]).toBeDefined();
    }

    /* 2. Only approved namespaces exist. Anything else is drift. */
    for (const key of sizes) {
      expect(APPROVED_FONT_SIZE_PREFIXES.some((prefix) => key.startsWith(prefix))).toBe(true);
    }

    /* 3. The spot checks M66.1 wrote, unchanged. */
