...
describe('M66.7 — ROUTE TRUTH (CTO decision D-6 A)', () => {
  ...
  const REAL_ROUTES = [
    ...
    '/terms',
    '/source-policy',
    // SUPPORT CLOSURE — a real App Router page (frontend/src/app/support/page.tsx),
  ...
  it('every rendered destination is a real, existing route', () => {
    // SUPPORT CLOSURE — FOUR, not three. The count stays EXACT: its value is
    // that it fails when an UNVETTED destination appears, and it did exactly
...
describe('M66.7 — BEHAVIOUR PRESERVED (CTO decision D-1 A)', () => {
  ...
  it('DEFECT-050 is not reproduced: a real <footer> landmark with a labelled <nav>', () => {
    ...
    expect(code).toMatch(/<\/footer>/);
    expect(code).toMatch(/<nav\s[^>]*aria-label=/);
  });

  /**
   * CTO decision D-4 A, AS AMENDED BY THE SUPPORT CLOSURE AUTHORIZATION.
   *
   * D-4 A originally required the nav's accessible name to be an
   * EXISTING dictionary string, and `t.groupTitles.Legal` was the one
   * chosen. That was accurate while Legal was the only link group. The
   * Support closure adds a second group, so naming the whole navigation
   * after one of its groups became wrong -- a screen-reader user was
   * told the landmark was "Legal" and then handed Help links inside it.
   *
   * The CTO authorized exactly ONE new key for this, and no more:
   * `footer.navigationAriaLabel` -- EN "Footer links", PL "Linki w
   * stopce". Every OTHER property D-4 A asserted is preserved below and
   * is still asserted: concise, under 24 characters, localized in both
   * languages, genuinely translated rather than copied, and never the
   * tagline. Only the source of the string moved.
   */
  it('CTO decision D-4 A (amended) -- the nav accessible name is a concise localized string', () => {
    expect(code).toMatch(/aria-label=\{t\.navigationAriaLabel\}/);
    expect(code).not.toMatch(/aria-label=\{t\.tagline\}/);
    // The whole navigation is no longer named after one of its groups.
    expect(code).not.toMatch(/aria-label=\{t\.groupTitles\.Legal\}/);
    for (const language of ['en', 'pl'] as const) {
      const label = getDictionary(language).footer.navigationAriaLabel;
      expect(label.length).toBeGreaterThan(0);
      expect(label.length).toBeLessThan(24);
    }
    expect(getDictionary('pl').footer.navigationAriaLabel).not.toBe(
      getDictionary('en').footer.navigationAriaLabel,
    );
  });
...
describe('M66.7 -- copy (CTO decision D-4 A, amended): exactly one key was added', () => {
  ...
  it('every visible string is localized in both languages', () => {
    // `navigationAriaLabel` is the ONE key the Support closure was
    // authorized to add. It is asserted here alongside the pre-existing
    // strings so that the exception is visible in the record rather than
    // implied by a test that no longer says what it once said.
    for (const language of ['en', 'pl'] as const) {
      ...
      expect(t.navigationAriaLabel.length).toBeGreaterThan(0);
    }
    ...
    expect(pl.navigationAriaLabel).not.toBe(en.navigationAriaLabel);
