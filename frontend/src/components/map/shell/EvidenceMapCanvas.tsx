    ...
    <div className="relative h-full w-full" data-gn="map-canvas" data-gn-density={density}>
      ...
      */}
      ...
      <div data-gn="map-labels" aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
        {labels.map((label) => {
          /*
            D-2 added `lake`, `river` and `city` candidates, so the type is now
            looked up by kind rather than decided by a three-branch ladder that
            silently fell through to `country`. A new kind that forgets its
            type entry is a TypeScript error here, not a city drawn at country
            size.
          */
          ...
          return (
            <span
              ...
              className={`absolute -translate-x-1/2 -translate-y-1/2 whitespace-nowrap font-gn-mono ${
                /*
                  WATER INK FOR WATER, WHATEVER ITS SIZE. A lake and a river
                  are the same class of fact as an ocean, so they take the same
                  italic blue-slate the reference gives the sea — which is also
                  how a reader tells at a glance that a label names geography
                  and not a place where something happened.
                */
                label.kind === 'water' || label.kind === 'lake' || label.kind === 'river'
                  ? 'italic text-[#4b7f92]'
                  : label.kind === 'continent'
                    ? 'text-[rgba(139,161,174,.55)]'
                    : /* A settlement is quieter than the country that contains it. */
                      label.kind === 'city'
                      ? 'text-[#93a7b4]'
                      : 'text-[#a8bcc8]'
