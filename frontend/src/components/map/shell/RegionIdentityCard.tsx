...
const Row = ({ heading, children }: { heading: string; children: React.ReactNode }) => (
  <div className="mt-3 border-t border-gn-line-structural pt-3">
    <p className="text-[10px] uppercase tracking-[0.14em] text-sp-ink-dim">{heading}</p>
...
export function RegionIdentityCard({
  ...
  if (region === null) {
    return (
      <div data-gn="region-card" data-gn-region-resolved="false" className="text-sp-ink">
        ...
        <p className="mt-1 text-[12px] leading-[1.5] text-sp-ink-dim">{labels.unresolvedBody}</p>
        {/* Verbatim, and in a code face, so it reads as an identifier rather than a name. */}
        <p data-gn="region-id" className="mt-2 break-all font-mono text-[11px] text-sp-ink-dim">
        ...
        {onClearSelection && (
          <button
            ...
            className="mt-3 min-h-[44px] w-full rounded-sm border border-gn-line-structural px-3 text-[12px] text-sp-ink transition-colors hover:bg-sp-surface-raised"
          >
            {labels.clear}
          </button>
        )}
  ...
  return (
    ...
    >
      <p className="text-[10px] uppercase tracking-[0.14em] text-sp-ink-dim">{labels.heading}</p>
      <p data-gn="region-name" className="mt-1 text-[15px] font-medium leading-[1.25]">
        {region.name}
      </p>
      <p data-gn="region-type" className="mt-1 text-[11px] leading-[1.45] text-sp-ink-dim">
      ...
      <Row heading={labels.definitionHeading}>
        ...
        */}
        ...
        {region.definitionId === null && (
          <p data-gn="region-definition-absent" className="mt-1 text-sp-ink-dim">
      ...
      */}
      <p data-gn="region-no-boundary" className="mt-3 text-[11px] leading-[1.45] text-sp-ink-dim">
        {labels.noBoundary}
      </p>
      {!capability.cameraTargetDerivable && (
        <p data-gn="region-camera-held" className="mt-1 text-[11px] leading-[1.45] text-sp-ink-dim">
          {labels.cameraHeld}
        </p>
      )}
      {/* Not a disabled button. A control the product cannot honour is not drawn. */}
      <p data-gn="region-watch-unavailable" className="mt-1 text-[11px] leading-[1.45] text-sp-ink-dim">
