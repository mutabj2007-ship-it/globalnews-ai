...
export function CountryPanel({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/*
        THE LABEL AND THE DESTINATION DISAGREED.

        This said "View full country coverage" and navigated to
        `/search?q=<country>`, which does not list coverage — it RUNS AN
        ANALYSIS of the country name. Meanwhile the panel above already
        renders `response.articles` in full, with no truncation, so the
        full coverage was already on screen and the control promised to go
        and fetch it.

        The destination is genuinely useful, so it is kept and the label is
        corrected to name what actually happens. NOTHING about the
        navigation changes, and `viewFullCoverage` is left in the
        dictionaries untouched because the no-coverage sentence above still
        uses that wording.
      */}
      <button
        type="button"
