...
describe('GEO-PRECISION-1 — geographic precision terminology', () => {
  /**
   * THE LABEL LIVES IN TWO PLACES and both had the same wrong word. Missing
   * one would leave the claim on screen in the Executive Brief while the
   * geographic card had been corrected.
   */
  const PRECISION_PATHS = [
    {
      path: 'analysisWorkspace.brief.precision',
      read: (language: 'en' | 'pl') => getDictionary(language).analysisWorkspace.brief.precision,
    },
    {
      path: 'analysisWorkspace.geography.precision',
      read: (language: 'en' | 'pl') =>
        getDictionary(language).analysisWorkspace.geography.precision,
    },
  ] as const;

  it('BOTH English retrieval labels state RETRIEVAL, not precision', () => {
    /*
     * city and country deliberately COLLAPSE to one tag. Both are derived from
     * retrievalContext, which is query-side by contract, so both describe where
     * we looked. Granularity is carried by the place name beside the tag; a tag
     * that varied by granularity would imply we had measured it.
     */
    for (const { path, read } of PRECISION_PATHS) {
      expect(`${path}.city: ${read('en').city}`).toBe(`${path}.city: RETRIEVED FOR`);
      expect(`${path}.country: ${read('en').country}`).toBe(`${path}.country: RETRIEVED FOR`);
    }
  });

  it('BOTH Polish retrieval labels carry the same meaning and are really translated', () => {
    for (const { path, read } of PRECISION_PATHS) {
      expect(`${path}.city: ${read('pl').city}`).toBe(`${path}.city: POZYSKANO DLA`);
      expect(`${path}.country: ${read('pl').country}`).toBe(`${path}.country: POZYSKANO DLA`);
      expect(read('pl').city).not.toBe(read('en').city);
      expect(read('pl').country).not.toBe(read('en').country);
    }
  });

  it('NO RETRIEVAL label in either language claims evidence, certainty, or anchoring', () => {
    const FORBIDDEN = /anchor|evidence|confirmed|verified|zakotwicz|dowod|potwierdz/i;

    /*
     * SCOPED TO THE RETRIEVAL KEYS, AND THAT IS A STRENGTHENING, NOT A RELAXATION.
     *
     * city and country describe where we looked and must never borrow the
     * vocabulary of what the evidence shows — that is what this regex protects.
     * `unresolved` deliberately DOES name evidence location, in order to say it
     * is UNRESOLVED. A regex cannot tell a claim from a denial, so scanning it
     * here would forbid the honest wording. It is pinned to its exact approved
     * value in the next test instead, which is a stronger assertion than a
     * negative match, not a weaker one. Every key remains asserted.
     */
    for (const { path, read } of PRECISION_PATHS) {
      for (const language of ['en', 'pl'] as const) {
        for (const key of ['city', 'country'] as const) {
          const value = read(language)[key];
          expect(typeof value).toBe('string');
          expect(`${path}.${key} [${language}]: ${value}`).not.toMatch(FORBIDDEN);
        }
      }
    }
  });

  it('the explicit denial of subnational evidence precision is PRESERVED in both languages', () => {
    // This line is the honest half of the geographic card and must survive
    // every future wording change to the label above it.
    expect(getDictionary('en').analysisWorkspace.geography.noSubnationalPrecision).toBe(
      'No subnational precision in evidence',
    );

    const pl = getDictionary('pl').analysisWorkspace.geography.noSubnationalPrecision;
    expect(pl.length).toBeGreaterThan(0);
    expect(pl).not.toBe(getDictionary('en').analysisWorkspace.geography.noSubnationalPrecision);
  });

  it('the UNRESOLVED label names evidence location and says it is unresolved', () => {
    for (const { path, read } of PRECISION_PATHS) {
      expect(`${path}.unresolved [en]: ${read('en').unresolved}`).toBe(
        `${path}.unresolved [en]: EVIDENCE LOCATION \u00b7 UNRESOLVED`,
      );
      expect(`${path}.unresolved [pl]: ${read('pl').unresolved}`).toBe(
        `${path}.unresolved [pl]: LOKALIZACJA W DOWODACH \u00b7 NIEUSTALONA`,
      );
    }
  });

  it('the AI-derived location label is labelled AI-derived and unverified', () => {
    /*
     * G-3. Never promoted to an evidence claim by agreeing with the retrieval
     * tag: agreement between the query and the model is not evidence.
     */
    expect(getDictionary('en').analysisResultView.aiInterpretedUnverified).toBe(
      'AI-INTERPRETED LOCATION \u00b7 UNVERIFIED',
    );
    expect(getDictionary('pl').analysisResultView.aiInterpretedUnverified).toBe(
      'LOKALIZACJA WG AI \u00b7 NIEZWERYFIKOWANA',
    );
  });

  it('G-1 EVIDENCE LOCATION vocabulary is ABSENT until a writer exists', () => {
    /*
     * NewsArticle.geographicPrecision has zero writers. A label nothing can set
     * would be wired to something by the first person who sees it, so the
     * evidence-resolved state ships WITH its writer or not at all. This test
     * fails the day someone adds the string alone.
     */
    for (const language of ['en', 'pl'] as const) {
      const precision = getDictionary(language).analysisWorkspace.geography.precision;
      expect(Object.keys(precision).sort()).toEqual(['city', 'country', 'unresolved']);
    }
  });
});
