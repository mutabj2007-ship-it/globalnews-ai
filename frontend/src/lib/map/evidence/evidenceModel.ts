...
export interface EvidenceTotals {
  ...
  readonly verifiedReportCount: number;
  /**
   * ── DISTINCT PUBLISHERS, OR NULL WHEN THE PRODUCER CANNOT SAY ────────────
   *
   * PO golden-frame correction: the rail's second total is SOURCES, not
   * "Records". Those are different facts — `recordCount` is how many evidence
   * rows came back, and one outlet can produce many — so the label could not
   * simply be changed over the old number without turning a true number into a
   * false claim about source diversity.
   *
   * Counted from `publisherId`, which this model already carries for exactly
   * this reason: `NewsArticle.sourcesCount` is hard-coded to 1 by all three
   * providers, so summing it reports "1 SOURCE" for a geography five outlets
   * covered.
   *
   * NULL, NEVER ZERO, when no record carries a publisher. Zero would assert
   * that nothing was published; null says the producer did not tell us, and
   * the rail renders those two differently.
   */
  readonly sourceCount: number | null;
  /**
   * ── DISTINCT SITUATIONS, OR NULL WHILE THE PRODUCER IS GATED ─────────────
   *
   * The golden's third total is SITUATIONS. PO ruling, verbatim: "If Situation
   * producers remain gated, render the truthful state — never a synthetic
   * non-zero situation count."
   *
   * Null whenever no record carries `situationIds` at all — which is today's
   * state, because the SITUATIONS mode is unbuilt and nothing populates the
   * field. The rail shows that as an explicit unavailable state rather than as
   * a confident 0, which would be the same lie in the other direction.
   */
  readonly situationCount: number | null;
}
...
export function evidenceTotals(records: readonly EvidenceRecord[]): EvidenceTotals {
  ...
  /*
    "Did ANY record carry this field?" is a different question from "how many
    distinct values were there", and only the first tells an empty answer apart
    from an absent producer. Both counts below depend on that distinction.
  */
  let anyPublisherSupplied = false;
  let anySituationSupplied = false;
  ...
  for (const record of records) {
    ...
    if (record.publisherId !== undefined) {
      anyPublisherSupplied = true;
      publishers.add(record.publisherId);
    }

    if (record.situationIds !== undefined) {
      anySituationSupplied = true;
      for (const situationId of record.situationIds) situations.add(situationId);
    }
  }
  ...
  return {
    ...
    newSinceLastVisit,
    verifiedReportCount,
    sourceCount: anyPublisherSupplied ? publishers.size : null,
    situationCount: anySituationSupplied ? situations.size : null,
  };
}
