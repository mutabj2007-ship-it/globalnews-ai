...
describe('3 · the resolver wraps, and must keep doing so after PO ruling A', () => {
  it('a longitude wound past the antimeridian resolves the same country', () => {
    /*
     * The camera contract used to be "SIGNED, UNWRAPPED, UNCLAMPED", and PO
     * ruling A (C906) replaced it with a canonical [-180, 180) domain — so a
     * -330° camera should no longer reach this function at all.
     *
     * THE ASSERTION STAYS ANYWAY, and deliberately. This resolver is called
     * with whatever the shell holds, and defence that costs one modulo is
     * exactly the defence worth keeping across a contract change: if a
     * non-canonical longitude ever reaches it again, it answers with the
     * country the reader is over rather than silently with "ocean".
     */
