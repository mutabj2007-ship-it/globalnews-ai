...
export function decodeCamera(raw: string | null | undefined): CameraState | null {
  ...
  /*
   * ── LONGITUDE IS WRAPPED, NOT REJECTED AND NOT CLAMPED — PO RULING A ─────
   *
   * Two superseded positions are worth keeping visible, because C906 is not a
   * return to either:
   *
   *   1. "A longitude of 999 is a malformed link, reject it." That was sound
   *      while the engine clamped inside one world.
   *   2. "Any finite longitude is an ordinary position, keep it." That was
   *      sound while the camera could genuinely travel — and it is what put
   *      cam=0.6/-1889/85 in a shipped Alpha URL.
   *
   * C906 takes neither. An out-of-range longitude is neither malformed nor
   * ordinary: it is a REAL POSITION SPELLED NON-CANONICALLY, and
   * `normaliseCamera` below wraps it into [-180, 180). So -1889 restores as
   * -89 — over the Americas, which is where that reader actually was — rather
   * than as a rejected link or as an empty ocean.
   *
   * That is the ruling's "reload of the URL reproduces the same sane view",
   * and it also means every Alpha URL already in someone's history heals on
   * open instead of restoring the failure.
   *
   * LATITUDE IS STILL RANGE-CHECKED, because ±90 is not a convention: there is
   * no position beyond the pole to restore, so a value outside it is malformed
   * in a way an out-of-range longitude is not.
   */
