...
export function allocateHomeFeed(
  ...
  /*
    VISIBLE-SURFACE EXCLUSIVITY — one story is shown once.

    `latestUpdates` used to be the whole article array. Every role above draws
    from the same fetch, so the hero live feed re-showed exactly the stories
    GlobalDevelopments had already published as `featured`/`inFocus`, and
    `discovery` on top of that. Measured on the live Alpha: 9 of 9 stories in
    the trending surface appeared again in the hero rail. Nothing was duplicated
    in the data — the same records were simply allocated to two visible
    presentations at once.

    The pool is drained in role order, so an undersupplied day now SHOWS FEWER
    CARDS rather than filling the rail by repetition. That is the intended
    trade: a short honest rail beats a long one that says the same thing twice.

    Keyed on `allocationKey` — story identity, not `id` — for the same reason
    the roles above are: one story can arrive as two records with two ids.
  */
  const latestUpdates = articles
    .filter((article) => !usedKeys.has(allocationKey(article)))
