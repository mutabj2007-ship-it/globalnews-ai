...
describe('getHomeFeed (Milestone #51 Phase B — single-request architecture)', () => {
  ...
  it('calls fetchTopHeadlines exactly once per invocation, with limit=24 (one request, sized for two exclusive surfaces)', async () => {
    const now = Date.now();
    const response: NewsResponse = {
      articles: Array.from({ length: 24 }, (_, i) => makeArticle(`a${i}`, new Date(now - i * 60_000).toISOString())),
      totalResults: 12,
      providers: ['gnews'],
      dataMode: 'live',
    ...
    expect(spy).toHaveBeenCalledWith(24, 'pl');
  });
  ...
  it('forwards the requested language through to the single backend request', async () => {
    ...
    expect(spy).toHaveBeenCalledWith(24, 'en');
  });
  ...
  it('returns the four explicit semantic roles derived from that one response', async () => {
    ...
    /* 24 fetched - 12 taken by the rail's roles = 12 left for the hero feed,
       and no story is in both. */
