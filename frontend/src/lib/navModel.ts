 ...
 *
 *     H-PUBLIC-NAV-QUOTA-SAFETY-1 — TEMPORARY SUSPENSION, STATED PLAINLY:
 *     the six editorial entries above are, as of this patch, NOT rendered
 *     as 'search'. Every one of them now carries kind 'unavailable' and NO
 *     href. The reason is quota, not design: /search?q=<term> is the real
 *     Analysis entry path, so a single click on a public header category
 *     used to start a real AI analysis for an anonymous visitor. Until the
 *     editorial surface is funded, those entries must cost nothing.
 *
 *     The 'search' kind is deliberately KEPT in the union below rather than
 *     deleted, because this is a suspension and not a removal: restoring
 *     the editorial nav is a one-line-per-entry change back, with the
 *     prototype's own exact query terms preserved in this comment — world,
 *     politics, economy, technology, science+research, health.
 *
...
export const NAV_MODEL: NavModelEntry[] = [
  ...
  { label: 'World', labelKey: 'world', kind: 'unavailable' },
  { label: 'Politics', labelKey: 'politics', kind: 'unavailable' },
  { label: 'Business', labelKey: 'business', kind: 'unavailable' },
  { label: 'Technology', labelKey: 'technology', kind: 'unavailable' },
  { label: 'Science', labelKey: 'science', kind: 'unavailable' },
  { label: 'Health', labelKey: 'health', kind: 'unavailable' },
