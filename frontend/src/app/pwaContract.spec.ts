...
describe('PWA-1 §5 — M66.13 localization is preserved exactly', () => {
  ...
  it('the three application-wide faces have not quietly stopped being preloaded', () => {
    /*
      This replaces a count — "there are exactly three `preload: false`" —
      which was a fact about how many faces existed in M66.1 rather than a
      property of the PWA. It broke as soon as an approved design added an
      isolated face, and it never checked the thing that matters here: that
      the three faces EVERY route renders on are still preloaded. A route-
      scoped face opting out costs nothing; one of these opting out changes
      first paint on every screen in the product, installed or not.
    */
    for (const [name, family] of [
      ['displayFont', 'Space_Grotesk'],
      ['bodyFont', 'Inter'],
      ['monoFont', 'IBM_Plex_Mono'],
    ] as const) {
      const start = layoutCode.indexOf(`const ${name} = ${family}({`);
      expect(start).toBeGreaterThan(-1);
      const body = layoutCode.slice(start, layoutCode.indexOf('});', start));
      expect(body).not.toMatch(/preload:\s*false/);
    }
  });

  it('route-scoped font additions leave the offline and precache contract alone', () => {
    /*
      next/font/google self-hosts at build time: the families are emitted as
      static assets on this origin and no request ever leaves for a font CDN.
      That is precisely why adding a face must not introduce a font origin
      anywhere in the service worker, the offline page or the manifest — if
      one ever appears, an installed app has taken on a third-party network
      dependency it cannot satisfy offline.
    */
    const FONT_ORIGINS = ['fonts.googleapis.com', 'fonts.gstatic.com', 'use.typekit', 'cdn.jsdelivr'];
    const swSource = readFileSync(join(publicDir, 'sw.js'), 'utf-8');
    const offlineSource = readFileSync(join(publicDir, 'offline.html'), 'utf-8');

    for (const surface of [swSource, offlineSource, manifestRaw]) {
      for (const origin of FONT_ORIGINS) {
        expect(surface).not.toContain(origin);
      }
    }

    /* And the layout itself reaches fonts only through next/font/google. */
    for (const origin of FONT_ORIGINS) {
      expect(layoutSource).not.toContain(origin);
    }
  ...
  it('mounts the service-worker registrar exactly once, and nothing else', () => {
    ...
    // whitespace. What remains must be the registrar, {children}, and the Ask
    // AI dock, in that order, and nothing else — no wrapper, no provider, no
    // second mount.
    //
    // ── WHY THIS EXPECTATION GREW BY ONE ELEMENT ────────────────────────
    //
    // ASK AI PHASE 1 mounts <AskAiDock /> here, and this assertion is updated
    // rather than relaxed. The two hazards it was written to catch are both
    // still caught, and they are the ones its own comment above names: a
    // SECOND registrar mount (still pinned to exactly one), and a WRAPPER
    // ELEMENT AROUND {children} that would put something between <body> and
    // the Claude Design tree.
    //
    // The dock is neither. It is a SIBLING that follows {children}, so nothing
    // is interposed above the Design tree, and it renders a fixed-position
    // control that enters no released GN-CD geometry. The equality is kept
    // EXACT — not loosened to a substring or a regex — so the next unplanned
    // addition to <body> still fails here, which is the whole value of the
    // lock.
