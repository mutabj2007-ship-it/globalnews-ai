  ...
  theme: {
    extend: {
      colors: {
        ...
        /* ── H2B · GN-CD-ANALYSIS-WORKSPACE + R1 ─────────────────────────────
         * ADD, NEVER REDEFINE. Every name below is new, so it has zero
         * existing consumers and cannot change the appearance of any surface
         * that does not explicitly opt in — the same architecture M66.1
         * established for the cd-* family, applied again here.
         *
         * The Analysis Workspace does NOT reuse the cd-* family: it is a
         * separate released token set with a different page base (#05080d vs
         * #04060c), a different grid (64px vs 56px) and its own type scale.
         * Values are quoted from 01-MASTER-DESIGN-SPEC §2 and
         * 05-COLOR-HUD-TOKENS §2/§3, with the six semantic hues exactly as
         * R1's colour errata leaves them (no hue added, removed or re-mapped).
         */
        'gn-page': '#05080d',
        'gn-bar': 'rgba(6,10,16,.94)',
        'gn-panel': '#080d14',
        'gn-elevated': '#0a1119',
        'gn-chip': '#0c141d',

        'gn-line-structural': '#101923',
        'gn-line-card': '#16202e',
        'gn-line-strong': '#1b2634',
        'gn-line-chip': '#22303f',
        'gn-line-inert': '#1e2b39',
        'gn-line-hover': '#33465c',

        'gn-title': '#e6edf6',
        'gn-ink-primary': '#f1f6fb',
        'gn-ink-body': '#dfe9f4',
        'gn-ink-prose': '#d5e1ee',
        'gn-ink-secondary': '#93a7bd',
        'gn-ink-tertiary': '#7d92aa',
        'gn-ink-active': '#eef4fa',
        'gn-ink-hover': '#c7d5e4',
        'gn-hud': '#54687f',
        'gn-hud-faint': '#4a5c73',
        'gn-disabled': '#3d4d61',
        ...
        /* The six semantic hues — 05 §2. A hue may only carry its assigned meaning. */
        'gn-verified': '#34d399',
        'gn-ai': '#60a5fa',
        'gn-geo': '#22d3ee',
        'gn-significance': '#a78bfa',
        'gn-uncertain': '#f59e0b',
        'gn-provenance': '#94a3b8',

        /* 10% tints — 05 §4 alpha ladder, background band. */
        'gn-verified-tint': 'rgba(52,211,153,.10)',
        'gn-ai-tint': 'rgba(96,165,250,.10)',
        'gn-geo-tint': 'rgba(34,211,238,.10)',
        'gn-significance-tint': 'rgba(167,139,250,.10)',
        'gn-uncertain-tint': 'rgba(245,158,11,.10)',
        'gn-provenance-tint': 'rgba(148,163,184,.10)',

        /* H2C — 04-ELEMENT-SPECS surfaces and inks. ADD ONLY; nothing above is redefined. */
        'gn-telemetry': '#080e15',
        'gn-drawer': '#070c12',
        'gn-scrim': 'rgba(3,6,10,.72)',
        'gn-pill-hover': '#101a25',
        'gn-line-telemetry': '#17222f',
        'gn-line-pill': '#24333f',
        'gn-line-pill-hover': '#3c526a',
        'gn-line-kind': '#2a3a4d',
        'gn-line-brief': '#1b3555',
        'gn-verified-soft': '#6ee7b7',
        'gn-verified-faint': 'rgba(52,211,153,.05)',
        'gn-brief-label': '#93c5fd',
        'gn-ink-cell': '#dbe6f2',
        'gn-ink-card': '#eaf1f8',
        'gn-ink-quote': '#b8cbdc',
        'gn-ink-value': '#a9bccf',
        'gn-ink-strong': '#cbd5e1',
        'gn-ink-chip': '#e2e8f0',
        'gn-ink-disc': '#b9cadb',
        'gn-ink-kind': '#8ba3bd',
        'gn-ink-toggle': '#6f8aa5',
        'gn-ink-meta': '#5c718c',

        /* H2D — E-13/E-24/E-25/E-26/E-28/E-29. ADD ONLY. */
        'gn-geo-surface': '#071016',
        'gn-geo-wash': 'rgba(34,211,238,.07)',
        'gn-geo-header': '#67e8f9',
        'gn-geo-dim': '#4b7f8c',
        'gn-line-geo': 'rgba(34,211,238,.22)',
        'gn-line-geo-soft': 'rgba(34,211,238,.16)',
        'gn-ink-geo': '#eaf6fa',
        'gn-ink-geo-soft': '#93b6c2',
        'gn-ai-wash': 'rgba(96,165,250,.05)',
        'gn-line-ai': 'rgba(96,165,250,.24)',
        'gn-line-brief-rule': 'rgba(59,130,246,.16)',
        'gn-ink-watch': '#c3d3e4',
        'gn-ink-muted': '#6b7f96',
        /**
        ...
        /* ══════════════════════════════════════════════════════════════════
           C907 §1 — THE SPATIAL INTELLIGENCE CHROME TOKENS (`sp-*`)
           ══════════════════════════════════════════════════════════════════

           WHAT THIS FIXES. The Spatial shell has painted itself with 42
           distinct `sp-*` utility names since the base commit, and this theme
           has never defined one of them. Tailwind emits nothing for an unknown
           token, so every panel fill, rail ground, divider, border and accent
           ink on that surface produced NO CSS and fell through to
           `<body className="bg-void">` — `#080b12`, the near-black the CTO
           measured on Alpha. `tailwind.config.ts` is byte-identical across
           base/c900/c903/c905/c906, so this was never a regression; it never
           worked.

           ── PROVENANCE, PER VALUE, NO EXCEPTIONS ─────────────────────────

           The ruling is explicit: *"DO NOT invent an sp palette."* Not one
           value below was chosen by me. Each is quoted from one of exactly
           three accepted sources, and each carries its citation:

             ROOT   `02 Interactive Prototype.html` :root custom properties —
                    the same block `designRenderTokens.ts` was extracted from.
             CSS    a named selector in that prototype's own stylesheet.
             P4     `Part IV - Monetization Layering v1.2 R2` §19 NEW TOKENS,
                    and `15 Visual Specification - Monetization Layers.dc.html`
                    for the chip treatments §19 declares a base hue for.

           Where the prototype's :root already names a token, the `sp-*` name
           maps to it one-to-one and the hex is copied character for character.
           Cross-check: every value that also appears in the v1.5 luminance
           change log's authoritative table agrees with it — panel `#0C151C`,
           sections `#101B25`, divider `.22`, emphasis divider `.34`, rail
           border `.4`, ink `#E4EEF4` / `#9DB3C0` / `#64798A`. Two independent
           sources, one set of values.

           ── ADD, NEVER REDEFINE ──────────────────────────────────────────

           Every name here is new, so this has zero existing consumers beyond
           the Spatial surface that was already asking for it, and it cannot
           change the appearance of anything that does not opt in. No existing
           token moves. That is the same architecture `cd-*` was added under.

           THREE NAMES WERE NOT IN THE AUTHORITY AND ARE NOT INVENTED HERE.
           `sp-ink-4`, `sp-ink-dim` and `sp-surface-raised` had no counterpart
           in any accepted source. Rather than mint three colours, their
           consumers are repointed at the proven token that carries the same
           role — see the C907 report. No token is defined for them, and the
           gate fails if one of those names ever comes back.
        */
        sp: {
          /* ROOT `--bg`. The Spatial surface ground, beneath every panel. */
          bg: '#05090d',
          /* ROOT `--panel`. v1.5: "right rail background #0A1117 -> #0C151C". */
          panel: '#0c151c',
          /* ROOT `--panel-2`. v1.5: "sections #101B25". */
          'panel-2': '#101b25',
          /* CSS `#rail{background:#09121a}` — the left layer rail. */
          rail: '#09121a',

          /*
            THE THREE-STEP DIVIDER LADDER.

            v1.5 moved the first two (.16 -> .22 ordinary, .28 -> .34 emphasis)
            and named the third in its canvas/rail separation clause: "Right
            rail: rgba(126,166,186,.4) left border … Layer rail: matching .4
            border". CSS `#rail{border-right:1px solid rgba(126,166,186,.4)}`
            is that same value in the prototype. This is the line that makes
            the rail "sit above the map rather than beside it" — the exact
            separation the golden frames show and Alpha lost.
          */
          line: 'rgba(126,166,186,.22)',
          'line-2': 'rgba(126,166,186,.34)',
          'line-3': 'rgba(126,166,186,.4)',

          /* ROOT `--ink`, `--ink-2`, `--ink-3`, `--muted`. The full ink ladder. */
          ink: '#e4eef4',
          'ink-2': '#9db3c0',
          'ink-3': '#64798a',
          muted: '#4a5b67',
          ...
          /*
            THE TWO INTELLIGENCE HUES, and the ink that goes ON them.

            `cyan` and `amber` are the same values `colourGrammar.ts` and
            `designRenderTokens.ts` already carry — one hue, one hex, three
            modules. `cyan-on` / `amber-on` are the dark inks the prototype
            puts on a SOLID control of that hue, which is the one place a
            light ink would be unreadable.
          */
          cyan: '#3ad6e6',
          /* CSS `.act.primary{color:#04161a;background:var(--cyan)}`. */
          'cyan-on': '#04161a',
          /* CSS `.act.primary:hover{background:#68e6f2}`. */
          'cyan-hover': '#68e6f2',
          amber: '#f2a93c',
          /* CSS `.follow.on{color:#1a1102;background:var(--amber)}`. */
          'amber-on': '#1a1102',

          /* ROOT `--ui-idle`, `--ui-hover`, `--ui-off` — control states. */
          'ui-idle': '#b4c8d4',
          'ui-hover': '#e4eef4',
          'ui-off': '#5a6e7d',

          /* ROOT `--ocean`. Identical to `DESIGN_REFERENCE.ocean`, by design. */
          ocean: '#040a10',

          /*
            THE STORY CARD, which is the retained-reporting rail's whole
            visual identity. ROOT supplies the three surfaces and the two
            lines; the prototype's own `.card` rules supply the three inks.
          */
          story: '#10262f',
          'story-hover': '#163845',
          'story-selected': '#112a34',
          'story-line': 'rgba(94,158,178,.22)',
          'story-line-hover': 'rgba(94,158,178,.4)',
          /* CSS `.card .h{color:#e6f1f6}`. */
          'story-headline': '#e6f1f6',
          /* CSS `.card .m{color:#7d97a4}`. */
          'story-meta': '#7d97a4',
          /* CSS `.card .m b{color:#9db4c0}`. */
          'story-meta-strong': '#9db4c0',

          /* CSS `.item:hover{background:#101a24}` — the neutral raised hover. */
          'item-hover': '#101a24',
          /* CSS `#search{background:#101e28}` — the place-search field. */
          field: '#101e28',
          /* CSS `#top{background:linear-gradient(180deg,#0b141b,#080f15)}`. */
          'top-a': '#0b141b',
          'top-b': '#080f15',

          /*
            THE CATEGORY METER. Three values, three selectors, one control.
            CSS `.cat .tr{background:#131e27}`, `.cat .tr i{background:#2b6f7c}`,
            `.cat:hover .tr i{background:#3a8b99}`.
          */
          'track-bg': '#131e27',
          track: '#2b6f7c',
          'track-hover': '#3a8b99',
          ...
          /*
            PART IV — the three monetization hues. §19 NEW TOKENS declares the
            base of each verbatim:
              --watch: #5BE3A8   active watch / running assignment
              --capability: #8C86EE   tier boundary only
              --compute: #D8C08A   metered AI action

            The `-line` and `-dim` derivatives are NOT declared in §19, so they
            are taken from the recurring chip treatment in the Part IV visual
            specification rather than chosen. Counted across that document's
            inline styles: the mint chip is `1px solid rgba(91,227,168,.45)`
            over `rgba(91,227,168,.1)` — the most frequent border/background
            PAIR for that hue, and the treatment of the left-rail watchboard
            icon these consumers reproduce. Capability carries a border at
            `.5` and, in every occurrence, NO fill — which independently
            corroborates §19's own "tier boundary only". Compute's only
          ...
          */
          ...
          capability: '#8c86ee',
          'capability-line': 'rgba(140,134,238,.5)',
          compute: '#d8c08a',
          'compute-line': 'rgba(216,192,138,.45)',
        },
      },
      ...
      fontFamily: {
        ...
        /*
         * H2B — 01-MASTER-DESIGN-SPEC §3: two families only. Outfit for
         * intelligence content, JetBrains Mono for HUD/telemetry. The split is
         * itself semantic, so they are separate families rather than weights of
         * one. New variables, exactly as M66.1 did for cd-*: the legacy and
         * cd-* declarations are untouched, so no existing route restyles.
         */
        'gn-display': ['var(--font-gn-display)', 'sans-serif'],
        'gn-mono': ['var(--font-gn-mono)', 'monospace'],
      ...
      fontSize: {
        ...
        /* H2B — 01-MASTER-DESIGN-SPEC §3 type roles, verbatim. */
        'gn-title': ['27px', { lineHeight: '1.18', letterSpacing: '-0.022em', fontWeight: '600' }],
        'gn-title-m': [
          '18px',
        ...
        'gn-prose': ['13px', { lineHeight: '1.55', fontWeight: '400' }],
        'gn-hud-section': ['9px', { lineHeight: '1.2', letterSpacing: '0.18em' }],
        'gn-hud-label': ['9px', { lineHeight: '1.2', letterSpacing: '0.16em' }],
        'gn-hud-index': ['10.5px', { lineHeight: '1.2', letterSpacing: '0.09em' }],
        'gn-hud-meta': ['9px', { lineHeight: '1.3', letterSpacing: '0.10em' }],
        'gn-hud-chip': ['9px', { lineHeight: '1.2', letterSpacing: '0.08em' }],

        /* H2C — 04-ELEMENT-SPECS type roles, verbatim. ADD ONLY. */
        'gn-hud-micro': ['8.5px', { lineHeight: '1.2', letterSpacing: '0.14em' }],
        'gn-hud-cell': ['10px', { lineHeight: '1.2', letterSpacing: '0.10em' }],
        'gn-hud-entry': ['10px', { lineHeight: '1.2', letterSpacing: '0.12em' }],
        'gn-hud-value': ['11px', { lineHeight: '1.2' }],
        'gn-hud-brief': ['9.5px', { lineHeight: '1.2', letterSpacing: '0.16em' }],
        'gn-hud-toggle': ['9.5px', { lineHeight: '1.2', letterSpacing: '0.10em' }],
        'gn-hud-outlet': ['9.5px', { lineHeight: '1.2', letterSpacing: '0.05em' }],
        'gn-hud-disc': ['9px', { lineHeight: '1' }],
        'gn-hud-cell-label': ['9px', { lineHeight: '1.2', letterSpacing: '0.14em' }],
        'gn-hud-supports': ['9px', { lineHeight: '1.3', letterSpacing: '0.08em' }],
        'gn-hud-ordinal': ['10px', { lineHeight: '1.2', letterSpacing: '0.06em' }],
        'gn-brief-prose': ['16.5px', { lineHeight: '1.62', fontWeight: '400' }],
        'gn-brief-prose-t': ['15px', { lineHeight: '1.62', fontWeight: '400' }],
        'gn-brief-prose-m': ['13.5px', { lineHeight: '1.6', fontWeight: '400' }],
        'gn-claim': ['15px', { lineHeight: '1.5', fontWeight: '400' }],
        'gn-entity': ['15px', { lineHeight: '1.3', fontWeight: '600' }],
        'gn-cell-value': ['14px', { lineHeight: '1.45', fontWeight: '400' }],
        'gn-card-title': ['14.5px', { lineHeight: '1.35', fontWeight: '600' }],
        'gn-quote': ['13px', { lineHeight: '1.55', fontWeight: '400' }],

        /* H2D — R1 type roles. ADD ONLY. */
        'gn-hud-subview': ['9.5px', { lineHeight: '1.2', letterSpacing: '0.12em' }],
        'gn-watch': ['12.5px', { lineHeight: '1.45', fontWeight: '400' }],
        'gn-timeline': ['14px', { lineHeight: '1.45', fontWeight: '400' }],
        'gn-geo-country': ['14.5px', { lineHeight: '1.3', fontWeight: '600' }],
      ...
      borderRadius: {
        ...
        /* H2B — 01-MASTER-DESIGN-SPEC §2 radii. */
        'gn-control': '7px',
        'gn-cell': '10px',
        'gn-card': '11px',
        'gn-module': '12px',
      ...
      screens: {
        ...
        /*
         * H2B — 06-RESPONSIVE-BREAKPOINTS. Only the XL gate is new: the
         * design's 768 / 1024 / 1280 boundaries are already Tailwind's own
         * md / lg / xl defaults exactly, so adding aliases for them would be
         * three names for values that already exist.
         */
        'gn-xl': '1600px',

        /*
         * H-2 — DESIGN'S OWN SPATIAL BOUNDARY, 861px.
         *
         * RECOVERED VERBATIM FROM FORMAL C55 `3db5a09` FOR R5 (the trailing
         * paragraphs below are C55's own words; only this note is new).
         *
         * ITS ABSENCE FROM THIS LINEAGE WAS THE MEASURED CAUSE OF THE R4
         * GATE N DEFECT. `MapPageClient` already returns exactly ONE variant —
         * the legacy branch sits after `if (mapVariant === 'shell') return`
         * and is unreachable while the flag is on. But the two Spatial
         * compositions INSIDE the shell branch are separated by
         * `spatial:block` / `spatial:hidden`, and an UNREGISTERED screen name
         * compiles to no rule at all — so the desktop workspace stayed
         * `hidden` at every width while the mobile composition showed at every
         * width. Registering the name is the whole fix; no route, component or
         * selection logic changes.
         *
         * Part II puts the desktop composition at 861 and above: below 860 the
         * layer rail becomes a horizontal chip strip, the legend hides and the
         * bottom sheet takes over. The Spatial workspace was gated on `lg`
         * (1024), which is why the browser measured callout=false at 861 even
         * though CALLOUT_MIN_VIEWPORT_WIDTH is 861 — its own gate passed and
         * the workspace it lives in had not mounted.
         *
         * ADD, NEVER REDEFINE: a NEW name. `lg` is untouched and every existing
      ...
      backgroundImage: {
        ...
        /*
         * H2B — the workspace page texture, 01-MASTER-DESIGN-SPEC §2.
         * Structural HUD, not decoration: it establishes the 64px grid the
         * composition sits on. Carries no data and never reduces text contrast.
         */
        'gn-grid':
          'linear-gradient(rgba(58,110,165,.055) 1px, transparent 1px), linear-gradient(90deg, rgba(58,110,165,.055) 1px, transparent 1px)',
        /* H2D — E-13 surface grid, 22px cyan. ADD ONLY. */
        'gn-geo-grid':
      ...
      keyframes: {
        /* H2D — E-13 outer certainty ring. ADD ONLY. */
        'gn-pulse': {
          '0%, 100%': { opacity: '.55', transform: 'translate(-50%, -50%) scale(1)' },
          '50%': { opacity: '.15', transform: 'translate(-50%, -50%) scale(1.12)' },
        },
        // ── EXISTING — UNCHANGED. ────────────────────────────────────────
        'ring-pulse': {
      ...
      animation: {
        /* H2D — E-13. ADD ONLY. */
        'gn-pulse': 'gn-pulse 2.6s ease-in-out infinite',
