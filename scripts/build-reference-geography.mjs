...
const LAKE_ROWS: readonly ReferencePlaceRow[] = ${JSON.stringify(lakeSeeds.map(({ kind, ...rest }) => rest), null, 0)};

const RIVER_ROWS: readonly ReferencePlaceRow[] = ${JSON.stringify(riverSeeds.map(({ kind, ...rest }) => rest), null, 0)};

const CITY_ROWS: readonly ReferencePlaceRow[] = ${JSON.stringify(citySeeds.map(({ kind, ...rest }) => rest), null, 0)};
...
export const REFERENCE_PLACE_COUNTS = {
  lake: ${lakeSeeds.length},
  river: ${riverSeeds.length},
  city: ${citySeeds.length},
} as const;
`;
