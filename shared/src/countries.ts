...
export const COUNTRIES: CountryMeta[] = [
   ...
   * MADAGASCAR — RESTORED.
   *
   * It was absent from this table while the gazetteer held 22 of its admin1
   * regions, 6 of its settlements and a derived country extent. The
   * consequence was total: every one of those records is unreachable without a
   * row here, because a settlement whose ISO2 does not resolve to a country is
   * dropped rather than served with a hole in it. Madagascar could not be
   * searched, could not be resolved from an article, and was missing from
   * Eastern Africa's membership.
   *
   * A registry-completeness gate now fails the suite if any sovereign state the
   * GAZETTEER knows about is absent from this table - see
   * `country-registry-completeness.spec.ts`. That check reads a source
   * independent of this file, which is why it can catch an omission that every
   * COUNTRIES-derived assertion was structurally blind to.
   */
  { iso2: 'MG', iso3: 'MDG', isoNumeric: '450', name: 'Madagascar', region: 'Africa' },
...
const CITY_TO_ISO3: Record<string, string> = {
  ...
  /*
    G-SEARCH-CITY-COVERAGE-GAP-1 — TIER 1: PRIORITY-COUNTRY CAPITALS THAT WERE NEVER ADDED.

    MEASURED GAP. Six of the nine East Africa priority countries had ZERO
    curated cities: Uganda, Tanzania, Burundi, DR Congo, South Sudan and
    Somalia. A story naming only the capital -- which is how national news from
    these countries is usually written -- resolved to no country at all, so it
    never entered country retrieval and no amount of salience logic downstream
    could recover it.

    Meanwhile the table already carried Canberra, Ottawa and Pretoria, whose
    countries are not in any priority programme. The omission was not a policy;
    it was the incremental table never being audited against the priority set.

    WHERE A COUNTRY'S CAPITAL IS NOT ITS NEWS CENTRE, BOTH ARE CURATED. Tanzania
    moved its capital to Dodoma but its reporting still originates in Dar es
    Salaam; Burundi moved to Gitega in 2019 and most coverage still says
    Bujumbura. Curating only the de jure capital would have reproduced the gap
  ...
  */
  ...
  /*
    TIER 2 — NON-CAPITAL METROPOLISES OF COVERED PRIORITY COUNTRIES.

    The table was, in effect, a capitals table: 21 of its 24 entries were
    capitals. That is why "Stadium collapse in Marseille" and "summit in Lyon"
    resolved to nothing while "in Paris" resolved to France. A national event is
    routinely reported from the city it happened in, and for France, Poland and
    Kenya those cities are frequently not the capital.

    STRASBOURG IS DELIBERATE: it is the seat of the European Parliament and is
    named in exactly the Politics/EP material this programme already ingests.
  */
  marseille: 'FRA',
  lyon: 'FRA',
  toulouse: 'FRA',
  ...
  /*
    POLISH CITIES ARE CURATED IN BOTH SPELLINGS, AND THAT IS NOT OPTIONAL.

    normalizeGeographicLookupKey() collapses non-letters and lowercases, but it
    does NOT strip diacritics -- verified against the backend's own text
    normalizer, which behaves identically. So "Kraków" normalises to "kraków"
    and "Krakow" to "krakow": TWO DISTINCT KEYS. Curating only one spelling
    would silently resolve half of the real-world occurrences, and the half that
 ...
 */
/**
 * GEO-PRECISION-1 (B) — the lookup key for a curated geographic name.
 *
 * Lowercases, collapses every run of non-alphanumeric characters to a single
 * space, and trims. "Kigali," and "Kigali" therefore produce the same key,
 * and so do "New  Delhi" and "New Delhi".
 *
 * WHY THIS EXISTS, STATED PLAINLY SO IT IS NOT "SIMPLIFIED" AWAY LATER.
 * resolveCountryByCity() used to lowercase and trim only. The entirely
 * ordinary phrasing "Kigali, Rwanda" therefore failed the exact curated-city
 * lookup on the trailing comma alone. The query still resolved — but only
 * because resolveGeoTypo(), a LAST-RESORT TYPO CORRECTOR, scored "kigali," as
 * one insertion away from "kigali" and rescued it. Correct punctuation was
 * being reported back to the reader as a misspelling. Worse, that rescue is
 * unavailable to any curated city shorter than MIN_TARGET_LENGTH ("Rome",
 * "Kyiv") or written with a space ("New Delhi", "Addis Ababa"), so those
 * queries resolved to nothing at all.
 *
 * THIS DOES NOT WIDEN WHAT MATCHES. The key is still compared by exact
 * equality against the curated table. "Kigal" normalises to "kigal" and
 * "Kigali, Rwanda" normalises to "kigali rwanda"; neither is a curated key,
 * so both stay unresolved exactly as they were, and the tests that pin that
 * behaviour pass unchanged. This is punctuation tolerance, not fuzzy
 * matching — fuzzy matching lives in geo-fuzzy-resolver.ts and stays there.
 *
 * It is the same normalisation the backend's country-relevance layer already
 * applies to article text, so the two halves of the system finally agree on
 * what a place name looks like.
 */
export function normalizeGeographicLookupKey(input: string): string {
  return input
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]+/gu, ' ')
    .replace(/\s+/g, ' ')
...
export function resolveCountryByCity(input: string): CountryMeta | undefined {
  const key = normalizeGeographicLookupKey(input);
  if (!key) return undefined;

  const iso3 = CITY_TO_ISO3[key];
...
export function resolveLocationContext(input: string): LocationContext | undefined {
  ...
  /**
   * GEO-PRECISION-1 (B) — the CANONICAL key is carried forward, not the raw
   * input. This used to be `input.trim().toLowerCase()`, which was harmless
   * only for as long as the lookup itself rejected anything with punctuation
   * in it. Now that "Kigali," resolves, returning the raw form would put a
   * trailing comma into `city` — and from there into the provider search
